...
def list_all_annotations_url(args):
    ...
    for id_oai, record in dico_metadata.items():
        ...
        if subject == language_name and type_annotation == 'text' and url_diffusion.endswith('xml'):
            nb_annotations += 1 
           
            list_annotations.append(url_diffusion)  
            
    # print (list_annotations)
    rep_out = Path("results")
    rep_out.mkdir(exist_ok=True, parents=True)
    
    for xml_url in list_annotations:
       url_temp = xml_url.split('/')
       nom_fichier = url_temp[-1]
       
       print (xml_url)
       urllib.request.urlretrieve(xml_url, "results/"+nom_fichier)
